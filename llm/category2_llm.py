...
CATEGORY_NAME = os.path.join(PROJECT_ROOT, "data", "AI_Ethics")
...
QA_PROMPT = """
            Sen bir Yapay Zeka Etiği (AI Ethics) uzmanısın. Görevin, kullanıcının sorduğu soruları sana verilen belgelerdeki bilgiler ışığında dikkatle araştırmak ve anlamlı, kapsamlı, doğru cevaplar üretmektir.
            ...
            Aşağıda, AI_Ethics kategorisindeki belgelerden alınan ilgili bölümler yer alıyor:
            {context}
...
print("[INFO] Loading documents for AI_Ethics...")
...
def load_and_index_documents():
    """AI_Ethics kategorisi için dökümanları yükler ve FAISS indeksini oluşturur/yükler."""
    processor.get_or_create_index("AI_Ethics", DATA_DIR)
...
def get_qa_response(user_question: str, chat_history: list[dict] | None = None) -> dict:
    ...
    retriever = processor.get_retriever("AI_Ethics", DATA_DIR)
